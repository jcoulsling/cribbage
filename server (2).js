...
function freshRoom(code){
  ...
  return { code, soloPlay:true, sockets:Array(n).fill(null), names:Array(n).fill(""),
           ...
           last:null, idle:0, busy:false, log:[], chat:[], words:[] };
}
...
function settle(room, passed){
  ...
  const live = liveSeats(room);
  /* A pass hands you a card and the next turn to try it. The word is given up
     once everybody has had two turns without a card going down, so it works the
     same whether there is one of you or four. */
  if(passed) room.idle = (room.idle || 0) + 1;
  else room.idle = 0;
  const spent = room.idle >= Math.max(1, live.length) * 2;
